...
if page == "1. National AQI Overview":
    st.markdown('<p class="main-header">National Surface AQI Overview</p>', unsafe_type_html=True)
    st.markdown('<p class="sub-header">Sentinel-5P TROPOMI Satellite-Derived Ground-Level AQI Estimates over India</p>', unsafe_type_html=True)
...
# PAGE 2: HCHO HOTSPOT ANALYSIS
    ...
    st.markdown('<p class="main-header">HCHO Hotspot Cluster Analysis</p>', unsafe_type_html=True)
    st.markdown('<p class="sub-header">DBSCAN Unsupervised Clustering over elevated Satellite Formaldehyde Columns</p>', unsafe_type_html=True)
    
    col_sb1, col_sb2 = st.columns([1, 2])
    with col_sb1:
        st.markdown('<div class="hcho-card">', unsafe_type_html=True)
        ...
        st.markdown('</div>', unsafe_type_html=True)
...
# PAGE 3: BIOMASS BURNING ANALYSIS
    ...
    st.markdown('<p class="main-header">Biomass Burning vs. Satellite HCHO Correlation</p>', unsafe_type_html=True)
    st.markdown('<p class="sub-header">Investigating NASA MODIS / VIIRS crop burning inputs and its photochemical catalyst values</p>', unsafe_type_html=True)
    ...
    with col_t1:
        ...
        fig_line.update_layout(
            ...
            yaxis1=dict(title="Active MODIS Fire Counts", titlefont=dict(color="orange")),
            yaxis2=dict(title="HCHO density (mol/m²)", titlefont=dict(color="magenta"), overlaying="y", side="right"),
            height=400
        )
    ...
    with col_t2:
        ...
        st.markdown("<p style='text-align:center;'><b>Mathematical Correlation Coefficient R = 0.941</b> (Highly Significant)</p>", unsafe_type_html=True)

# PAGE 4: AI PREDICTION CENTER
elif page == "4. AI Prediction Center":
    st.markdown('<p class="main-header">AI Hotspot & AQI Prediction Center</p>', unsafe_type_html=True)
    st.markdown('<p class="sub-header">Evaluating machine learning regressors and hazard classification alert algorithms</p>', unsafe_type_html=True)
...
# PAGE 5: ANALYTICS & REPORTS
    ...
    st.markdown('<p class="main-header">Analytical Reports Hub</p>', unsafe_type_html=True)
    st.markdown('<p class="sub-header">Download automated chemical diagnostic papers and full geospatial architecture files</p>', unsafe_type_html=True)
