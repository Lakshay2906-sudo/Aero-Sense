...
// Health check endpoint can be accessed without keys loaded
const app = express();
const PORT = 3000;

app.use(express.json());

// Helper to safely obtain Gemini client dynamically
function getGeminiClient() {
  ...
  // Sanitize key intensely: trim whitespace, strip leading/trailing quotes or backticks, strip trailing semicolons
  apiKey = apiKey.trim()
    .replace(/^['"`\s]+|['"`\s]+$/g, '')
    .replace(/;+$/, '')
    .trim();

  // Guard against common environment placeholders or uninitialized state strings
...
app.post("/api/gemini/analyze", async (req, res) => {
  ...
  if (!apiKey) {
    return res.status(503).json({
      error: "Gemini API key is missing. Please define your GEMINI_API_KEY environment variable. Note: You should set it using the Secrets/Settings panel or inside your local environment configuration."
    });
  }
  ...
  if (!aiClient) {
    ...
    const preview = len > 8 ? `${trimmed.substring(0, 4)}...${trimmed.substring(len - 4)}` : trimmed;
    
    return res.status(503).json({
      error: `Your GEMINI_API_KEY has an invalid value format ("${preview}", character length: ${len}). Please click the Settings gear icon at the bottom-left of the Google AI Studio page, navigate to 'Secrets', update GEMINI_API_KEY with your active Google AI Studio API key (starts with "AIzaSy"), and restart/re-run the preview. Make sure to remove any quotes, backticks, or semicolons.`
    });
  }
...
Avoid listing generic markdown summaries. Ensure the response has deep, professional-grade technical terminology suitable for presentation at a national hackathon. Do not mention code files or API keys. Max 300 words.`;
    ...
    // Sequence of models to try in descending order of general stability and quota availability
    const modelsToTry = [
      "gemini-3.5-flash",
    ...
    if (!responseText) {
      throw lastError || new Error("All recommended Gemini models are currently experiencing heavy load. Please try again in safe intervals.");
    }

    console.log(`Success! Response generated using model: ${successModel}`);
